...
const addNewArticleHandler = async (req, res) => {
    const { title, description, text, cover } = req.body;
    ...
    if (!title || !description || !text) {
        ...
        return res.status(400).render('add-article.ejs', {
            ...
            theme: theme,
            user: req.user,
            errorMessage: 'Article is in need of title, description and text'
        });
    }
    ...
    try {
        ...
        await createArticleInDb({ title, description, text, cover, authorId });
